  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex justify-between items-center border-b border-gray-200 pb-4 mb-8 text-xs tracking-widest text-gray-500 uppercase">
        <span>{listProducts.length} PRODUCTS</span>
            ...
            {listProducts.map((product) => (
              <div key={product.id} className="group cursor-pointer">
                <div className="relative aspect-3/4 overflow-hidden bg-gray-100">
                  <img src={product.gambar} alt={product.nama} className="w-full h-full object-cover transition duration-500 group-hover:scale-105" />
                  <div className="absolute top-2 left-2 bg-red-500 text-white text-[10px] font-bold px-2 py-1 uppercase">SAVE RP {product.hargaCoret}</div>
                </div>
                <div className="mt-6 text-center">
                  <h4 className="text-[11px] font-bold tracking-widest text-stone-800 uppercase mb-2">{product.nama}</h4>
                  <div className="flex justify-center items-center gap-2">
                    <span className="text-xs text-red-600 font-bold">RP {product.harga}</span>
                    <span className="text-[10px] text-gray-400 line-through italic">RP {product.hargaCoret}</span>
                  </div>
                  <p className="text-[9px] text-gray-400 mt-2 uppercase tracking-[0.2em]">{product.kategori} COLORS AVAILABLE</p>
                </div>
              </div>
            ))}
          </div>
